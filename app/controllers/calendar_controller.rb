class CalendarController < ApplicationController
  ...
  def form_holiday(holiday)
    ...
    ret_var << '<td>' + (holiday.reason==1 ? (translate 'on_vacation') : holiday.reason==2 ? (translate 'ill_days') : holiday.reason==3 ? (translate 'buisness_trip') : holiday.reason==4 ? (translate 'remote_work') : holiday.reason==5 ? (translate 'meeting') : '') + '</td>' rescue '<td></td>'
    ret_var << '</tr>'
    ret_var << '</table>'
    return ret_var
  end

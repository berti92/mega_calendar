class CalendarController < ApplicationController
  ...
  def get_events
    ...
    @events = @events + holidays.collect {|h| {:title => h.user.login + ' - ' + (translate 'holiday'), :start => h.start.to_date.to_s, :end => h.end.to_date.to_s, :allDay => true, :color => def_holiday }}
    issues.each do |i|
      ...
      @events << {:title => i.id.to_s + ' - ' + i.subject, :start => i.start_date.to_date.to_s + tbegin, :end => i.due_date.to_date.to_s + tend, :color => color }
    end
